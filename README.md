...
| batch_size                     | Integer |            | (Default: 16MB) Maximum batch size in MB. At the end of each batch, the rows in the batch are loaded into Snowflake. |
| batch_wait_limit_seconds            | Integer |            | (Default: None) Maximum time to wait for batch to reach `batch_size`. |
